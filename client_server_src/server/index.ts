...
io.on('connection', (socket: any) => {
    ...
    socket.on('chat message', ({roomName, username, message}: {roomName: string, username: string, message: string}) => {
    ...
    // handle draw:start
    socket.on("draw:start", ({ roomName, x, y, color, size }: {roomName: string, x: number, y: number, color: string, size: string}) => {
        socket.to(roomName).emit("draw:start", { x, y, color, size });
    });

    // handle draw:move
    socket.on("draw:move", ({ roomName, x, y }: {roomName: string, x: string, y: string}) => {
        socket.to(roomName).emit("draw:move", { x, y });
     });

    // handle draw:end
    socket.on("draw:end", ({ roomName }: {roomName: string}) => {
